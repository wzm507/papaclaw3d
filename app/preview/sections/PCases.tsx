...
export default function PCases() {
  const sectionRef = useRef<HTMLElement>(null)
  const [active, setActive] = useState(0)

  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.utils.toArray<HTMLElement>('.p-case-row').forEach((row, index) => {
        gsap.fromTo(
          row,
          { y: 40, opacity: 0 },
          {
      ...
      <div className="p-inner">
        ...
        <div className="border-t border-[#E5E5E0]">
          {cases.map((item, index) => (
            <div
              key={item.title}
              className="p-case-row cursor-pointer border-b border-[#E5E5E0] transition-colors duration-300 hover:bg-[#F0EFEC]"
