...
export default function PPillars() {
  const sectionRef = useRef<HTMLElement>(null)

  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.utils.toArray<HTMLElement>('.p-pillar-row').forEach((row, index) => {
        gsap.fromTo(
          row,
          { y: 60, opacity: 0 },
          {
      ...
      <div className="p-inner">
        ...
        <div className="border-t border-[#E5E5E0]">
          {pillars.map((pillar) => (
            <div
              key={pillar.index}
              className="p-pillar-row group border-b border-[#E5E5E0] py-8 transition-colors duration-500 hover:bg-[#F0EFEC] md:py-12"
            >
